...
public class PunchCommand implements CommandExecutor {
	...
	public PunchCommand(Main plugin) {
		this.plugin = plugin;
	}
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		if(sender instanceof Player) {
			...
			if(target == null) {
				...
				if(sender.hasPermission("emoteme.punch")) {
					...
					cooldowns.put(sender.getName(), System.currentTimeMillis());
					Bukkit.broadcastMessage(MessageUtil.translate("&e" + sender.getName() + " &bhas &4punched&e " + target.getName() + "&b!"));
					return true;
				} else {
