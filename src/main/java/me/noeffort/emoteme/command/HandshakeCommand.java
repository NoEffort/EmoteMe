...
public class HandshakeCommand implements CommandExecutor {
	...
	public HandshakeCommand(Main plugin) {
		this.plugin = plugin;
	}
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		if(sender instanceof Player) {
			...
			if(target == null) {
				...
				if(sender.hasPermission("emoteme.handshake")) {
					int cooldowntime = 60;
					...
					cooldowns.put(sender.getName(), System.currentTimeMillis());
					Bukkit.broadcastMessage(MessageUtil.translate("&e" + sender.getName() + " &bshook hands with&e " + target.getName() + " &bo( ﾟ∇ﾟ)ﾉ゛"));
					return true;
				} else {
