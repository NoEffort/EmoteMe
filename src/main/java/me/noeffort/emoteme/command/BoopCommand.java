...
public class BoopCommand implements CommandExecutor {
	...
	public BoopCommand(Main plugin) {
		this.plugin = plugin;
	}
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		if(sender instanceof Player) {
			...
			if(target == null) {
				...
				if(sender.hasPermission("emoteme.boop")) {
					int cooldowntime = 60;
					...
					cooldowns.put(sender.getName(), System.currentTimeMillis());
					Bukkit.broadcastMessage(MessageUtil.translate("&e" + sender.getName() + " &bbooped&e " + target.getName() + " &b（・∀・）ゞ"));
					return true;
				} else {
