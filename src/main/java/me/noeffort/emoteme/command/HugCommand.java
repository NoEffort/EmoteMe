	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		if(sender instanceof Player) {
			...
			if(target == null) {
				...
				if(sender.hasPermission("emoteme.hug")) {
					int cooldowntime = 60;
					...
					cooldowns.put(sender.getName(), System.currentTimeMillis());
					if(target == sender) {
						Bukkit.broadcastMessage(MessageUtil.translate("&e" + sender.getName() + " &bis forever alone! 〴"));
						return true;
					} else {
						Bukkit.broadcastMessage(MessageUtil.translate("&e" + sender.getName() + " &bhas hugged&e " + target.getName() + " &b❤"));
						return true;
					}
				} else {
